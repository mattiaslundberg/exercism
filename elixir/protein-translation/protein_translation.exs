defmodule ProteinTranslation do
  ...
  defp proteins_from_rna(%{error: errors}) do
    {:error, Keyword.get(errors, :error)}
  end

  defp proteins_from_rna(%{ok: proteins}) do
    nans =
      proteins
      |> Keyword.values()
  ...
  def of_codon(codon) when not is_bitstring(codon), do: of_codon(to_string(codon))
  def of_codon(<<_, _, _>>), do: {:error, "invalid RNA"}
