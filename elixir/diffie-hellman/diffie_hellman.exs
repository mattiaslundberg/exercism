defmodule DiffieHellman do
  ...
  @spec generate_public_key(prime_p :: integer, prime_g :: integer, private_key :: integer) ::
          integer
  def generate_public_key(mod, n, k) do
    pow(mod, n, k)
  end
  ...
  def generate_shared_secret(prime_p, public_key_b, private_key_a) do
    pow(prime_p, public_key_b, private_key_a)
  end

  defp pow(mod, _, k) when k <= 0, do: 1

  defp pow(mod, n, k) when Integer.is_odd(k), do: Integer.mod(n * pow(mod, n, k - 1), mod)

  defp pow(mod, n, k) do
    res = pow(mod, n, div(k, 2))
    Integer.mod(res * res, mod)
  end
end
